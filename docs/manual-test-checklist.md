...
- [ ] With it on and keep-above set, clicking another window turns the assistant, after about a second, into keys plus labels, keys the same size, see-through between them.
...
- [ ] With it on, dragging a window edge or corner to resize works, including pausing briefly mid-drag (GNOME takes focus away for the drag).
